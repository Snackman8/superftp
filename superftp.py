...
def run(args):
    ftp_downloader = FtpFileDownloader(args['server'], args['username'], args['password'], args['connections'], args['port'],
                                       args['min_blocks_per_segment'], args['max_blocks_per_segment'])
    ftp_downloader.download_file(args['remote_path'], args['local_path'])            
    

def main():
    parser = argparse.ArgumentParser(description="Multi-segmented FTP downloader\n\nDownloads and FTP file using multiple threads concurrently.")
    ...
    parser.add_argument("--min_blocks_per_segment", help="minimum number of contigous 1MB blocks allocated per connection", type=int, default=8)
    parser.add_argument("--max_blocks_per_segment", help="maximum number of contiguous 1MB blocks per connection", type=int, default=128)
